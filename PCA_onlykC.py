...
class HFM(object):
    ...
    def __init__(self, t_data, x_data, y_data, Ci_data, Cb_data,
        ...
        self.net_cuvp = neural_net(self.t_data, self.x_data, self.y_data, self.kon_data, self.koff_data, layers=self.layers)
...
def start_train():
    ...
    # =============================================================================
    model.train(total_time=0.7, learning_rate=1e-3)
    # =============================================================================
    time_steps_test = 600
    ...
    for i in range(test_n):
        ...
        ax[2, 0].imshow(kon_loss.reshape(200, 200), cmap=plt.get_cmap('hot'))  # row=0,)  # row=1, col=0
        ...
        ax[2, 1].imshow(koff_loss.reshape(200, 200), cmap=plt.get_cmap('hot'))  # row=0,)  # row=1, col=0

        Ci_loss = Ci_test[j:j + 40000] - Ci_pred[j:j + 40000]
        ax[0, 2].imshow(Ci_test[j:j + 40000].reshape(200, 200))  # row=0, col=0
        ax[1, 2].imshow(Ci_pred[j:j + 40000].reshape(200, 200))  # row=0,)  # row=1, col=0
        ax[2, 2].imshow(Ci_loss.reshape(200, 200))  # row=0,)  # row=1, col=0

        Cb_loss = Cb_test[j:j + 40000] - Cb_pred[j:j + 40000]
        ax[0, 3].imshow(Cb_test[j:j + 40000].reshape(200, 200))  # row=0, col=0
        ax[1, 3].imshow(Cb_pred[j:j + 40000].reshape(200, 200))
        ax[2, 3].imshow(Cb_loss.reshape(200, 200))  # row=0,)  # row=1, col=0

        fig.suptitle(f'{j:.1f} ms', fontsize=14)
        ax[0, 0].set_title('kon test')
        ax[1, 0].set_title('kon pred')
        ax[2, 0].set_title('kon loss')

        ax[0, 1].set_title('koff test')
        ax[1, 1].set_title('koff pred')
        ax[2, 1].set_title('koff loss')

        ax[0, 2].set_title('Ci test')
        ax[1, 2].set_title('Ci pred')
        ax[2, 2].set_title('Ci loss')

        ax[0, 3].set_title('Cb test')
        ax[1, 3].set_title('Cb pred')
        ax[2, 3].set_title('Cb loss')
