...
class HFM(object):
    ...
    def __init__(self, t_data, x_data, y_data, Ci_data, Cb_data,
                 ...
                 layers, batch_size,
                 kon, koff, R0, D, Lv, Cv, SV, kon_data, koff_data):
        ...
        # data
        [self.t_data, self.x_data, self.y_data, self.Ci_data, self.Cb_data] = [t_data, x_data, y_data, Ci_data, Cb_data]
        [self.t_eqns, self.x_eqns, self.y_eqns] = [t_eqns, x_eqns, y_eqns]

        # placeholders
        [self.t_data_tf, self.x_data_tf, self.y_data_tf, self.Ci_data_tf, self.Cb_data_tf] = [
            tf.compat.v1.placeholder(tf.float32, shape=[None, 1]) for _ in range(5)]
        ...
        [self.kon_data_pred, self.koff_data_pred, *d1] = self.net_cuvp(self.x_data_tf,
                                                                    self.y_data_tf)

        # physics "informed" neural networks

        [self.Ci_eqns_pred,
         self.Cb_eqns_pred, *d2] = self.net_cuvp(self.t_eqns_tf,
                                                 self.x_eqns_tf,
                                                 self.y_eqns_tf)
        ...
        self.loss = mean_squared_error(self.Ci_data_pred+self.Cb_data_pred, self.Ci_data_tf+self.Cb_data_tf) + \
                    mean_squared_error(self.kon_data_pred, self.kon) + \
                    mean_squared_error(self.koff_data_pred, self.koff) + \
...
def start_train():
    ...
    model = HFM(t_data, x_data, y_data, Ci_data, Cb_data,
                ...
                kon, koff, R0, D, Lv, Cv, SV, kon_data, koff_data)
    # =============================================================================
    model.train(total_time=0.35, learning_rate=1e-3)
    # =============================================================================
    time_steps_test = 1000
    time_interval_test = 50

    np_input_array_test, num_items_test = get_data(time_steps_test, time_interval_test, u0, u, t0, Cb0, Cb)
