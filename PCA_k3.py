...
class HFM(object):
    ...
    def __init__(self, t_data, x_data, y_data, Ci_data, Cb_data,
        ...
        layersk = [2] + 10 * [4 * 10] + [2]
        self.net_cuvp_k = neural_net(self.x_data, self.y_data, layers=layersk)
        ...
        # ax[0].imshow(tf.reshape(self.kon_data_pred,[200,200], 1), cmap=plt.get_cmap('hot'))
        # ax[1].imshow(tf.reshape(self.kon_data_pred, [200, 200], 1), cmap=plt.get_cmap('hot'))
        # # ax[0, 1].imshow(self.Ci_data_pred.reshape(200, 200), cmap=plt.get_cmap('hot'))
        # # ax[1, 1].imshow(self.Cb_data_pred.reshape(200, 200), cmap=plt.get_cmap('hot'))
        # plt.show()
...
def start_train():
    ...
    model = HFM(t_data, x_data, y_data, Ci_data, Cb_data,
                ...
                kon, koff, R0, D, Lv, Cv, SV)
    # =============================================================================
    model.train(total_time=0.3, learning_rate=1e-3)
    # =============================================================================
    time_steps_test = 600
    ...
    kon_test = np.tile(kon,test_n).reshape(40000 * test_n, 1)
    koff_test = np.tile(koff, test_n).reshape(40000 * test_n, 1)
    ...
    for i in range(12):
        ...
        ax[0,0].set_title('{:.1f} ms'.format(j))
