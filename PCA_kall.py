...
def start_train():
    ...
    for i in range(test_n):
        ...
        kon_loss = kon_test[j:j + 40000] - kon_pred[j:j + 40000]
        im0 = ax[0, 0].imshow(kon_test[j:j + 40000].reshape(200, 200),  cmap=plt.get_cmap('summer'), vmin=kon_min, vmax=kon_max)  # row=0, col=0
        ax[0, 1].imshow(kon_pred[j:j + 40000].reshape(200, 200),  cmap=plt.get_cmap('summer'), vmin=kon_min, vmax=kon_max)  # row=0,)  # row=1, col=0
        ax[0, 2].imshow(kon_loss.reshape(200, 200),  cmap=plt.get_cmap('summer'), vmin=kon_min, vmax=kon_max)  # row=0,)  # row=1, col=0
        divider0 = make_axes_locatable(ax[0, 2])
        cax0 = divider0.append_axes("right", size="5%", pad=0.1)
        fig.colorbar(im0, cax=cax0)

        koff_loss = koff_test[j:j + 40000] - koff_pred[j:j + 40000]
        im1 = ax[1, 0].imshow(koff_test[j:j + 40000].reshape(200, 200),  cmap=plt.get_cmap('hot'), vmin=koff_min, vmax=koff_max)  # row=0, col=0
        ax[1, 1].imshow(koff_pred[j:j + 40000].reshape(200, 200), cmap=plt.get_cmap('hot'), vmin=koff_min, vmax=koff_max)
        ax[1, 2].imshow(koff_loss.reshape(200, 200), cmap=plt.get_cmap('hot'), vmin=koff_min, vmax=koff_max)  # row=0,)  # row=1, col=0
        divider1 = make_axes_locatable(ax[1, 2])
        cax1 = divider1.append_axes("right", size="5%", pad=0.1)
        fig.colorbar(im1, cax=cax1)

        Ci_loss = Ci_test[j:j + 40000] - Ci_pred[j:j + 40000]
        im2 = ax[2, 0].imshow(Ci_test[j:j + 40000].reshape(200, 200),  cmap=plt.get_cmap('ocean'), vmin=0, vmax=Ci_max)  # row=0, col=0
        ax[2, 1].imshow(Ci_pred[j:j + 40000].reshape(200, 200),  cmap=plt.get_cmap('ocean'), vmin=0, vmax=Ci_max)  # row=0,)  # row=1, col=0
        ax[2, 2].imshow(Ci_loss.reshape(200, 200),  cmap=plt.get_cmap('ocean'), vmin=0, vmax=Ci_max)  # row=0,)  # row=1, col=0
        ...
        ax[3, 2].imshow(Cb_loss.reshape(200, 200),  cmap=plt.get_cmap('bwr'), vmin=0, vmax=Cb_max)  # row=0,)  # row=1, col=0
        divider3 = make_axes_locatable(ax[3,2])
